...
if($actionString=="write"){
	...
	#---
	$sql = "
		insert into {{cms.tableNamePrefix}}_img_article (
			bda_title
			,bda_content
			,bd_seq
			,bda_fix_yn
	...
	$sql = "
		select count(*) as cnt
		from {{cms.tableNamePrefix}}_img_article_file
		where bda_seq = ${bdaSeq}
	";
	...
	#---
	$sql = "
		delete from {{cms.tableNamePrefix}}_img_article_file
...
function uploadFilesOfThisPage($bdaSeq=""){
	...
	for($bdaFileIndex=0;$bdaFileIndex<$bdaFileCount;$bdaFileIndex++){
		...
		if($uploadFileInfo!=null){
			...
			if($checkFileInfo==0){
				$sql = "
					insert into {{cms.tableNamePrefix}}_img_article_file (
						bda_seq
						,bdaf_filename
						,bdaf_save_filename
						,bdaf_save_thumbnail
				...
				$sql = "
					update {{cms.tableNamePrefix}}_img_article_file set
