...
#---
include($_SERVER["DOCUMENT_ROOT"].'/{{cms.prefix}}/inc/checkLoginApi.php');
#---
fnOpenDB();
#---
if($actionString=="GET_REPLY_LIST"){
	...
	$sql = "
		...
		from (
			select
				...
				,a.moddate
				,a.moduser
			from {{cms.tableNamePrefix}}_reply a
	...
	$sql = "
		select * 
		from {{cms.tableNamePrefix}}_reply
	...
	#---
	$sql = "
		insert into {{cms.tableNamePrefix}}_reply (
			bda_seq,
			bdr_content,
			regdate,
			reguser
	...
	#---
	$sql = "
		update {{cms.tableNamePrefix}}_reply set
	...
	#---
	$sql = "
		delete from {{cms.tableNamePrefix}}_reply
		where bdr_seq like '${bdrSeq}'
	";
	$affectedQueryCount = fnDBUpdate($sql);
	#---
